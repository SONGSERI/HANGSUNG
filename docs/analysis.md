...
## 6) 바로 실행 가능한 첫 분석 과제 (추천)

1. **Top 정지코드 10개의 시간손실 기여도 분석**
2. **설비별 가동률 vs 에러율 매트릭스 작성**
3. **부품(Part Number)별 에러 Pareto 분석**
4. **태그 스펙 이탈 상위 20개 태그 추출**

위 4가지는 현재 ERD만으로도 빠르게 구현 가능하며,
운영 개선 우선순위를 정하는 데 즉시 활용할 수 있다.
